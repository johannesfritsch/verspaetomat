...
Backend only, no mock equivalent yet: a journey holds customer, origin and destination station, the itinerary snapshot from Transitous at check-in (`itinerary`), the effective plan (`plan`, replaced from the transfer on after a re-plan), planned departure and arrival at the destination, status `riding | transfer | arrived | abandoned`, the proposed next leg while in transfer (`next_leg`) with its confirmation deadline, actual arrival, final delay at the destination, `missed_connection`, `incomplete`, points, ticket. A leg is a `rides` row with `journey_id`, `leg_no` and the transfer station it hands over at. Incidents carry `journey_id`; their evidence embeds the journey with its legs for the claim form. `GET /v1/me/destinations`, `GET /v1/journeys/plan`, `POST /v1/journeys`, `GET /v1/journeys/current`, `POST /v1/journeys/{id}/legs`, `POST /v1/journeys/{id}/finish`, `GET /v1/journeys`.
