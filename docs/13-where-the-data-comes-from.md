...
|---|---|---|---|
| "Du bist am Hauptbahnhof" (station nudge) | Your phone noticed you are standing at a station: the operating system wakes the app at a 300 m circle and the app checks for itself. Those fixes stay on the phone. What does leave it — on a journey as well as at rest — is one `GET /v1/stations/nearby` when the phone leaves the area it last looked at, gets a significant-location update outside it, or re-registers its zones from a list that belongs somewhere else. | iPhone: about 45 s after a fix lands within 50 m, and no nudge at all if none comes that close within six minutes. Android: one fix after a three-minute dwell, and the nudge goes out unless that fix puts you more than 300 m away. | Nothing happens. Open the app and pick the station yourself. |
| Which stations are near you, and the station search | A directory of every German railway station that the app carries with it and refreshes from verspaetomat.de about once a week. Your position is not sent for this. | The directory is cut about twice a year; the app checks weekly | The Bahnsteig says so and offers the search field. |
...
| Ticket screenshot on the claim | Attached by you from your photos or the camera; location and time tags are removed on the phone before it is sent. Kept only while the claim is open, in case the railway asks, then deleted. | — | The claim cannot be sent without it. |
...
| Your Verspätomat address (fahrgast-4711@users.verspaetomat.de) | Created by us at your first claim. Your name is the sender name. Every mail out gets copied to your private inbox; every mail in is forwarded to it whole. | — | — |
