...
`GET /v1/me/geofence` → all nine keys the handler returns: `{enabled, idle, last_checkin, stations: [{id, name, lat, lon, checkins}], quiet_from, quiet_to, snooze_until, stations_local, flags}`. `flags` (#41) is this customer's own answer for every wire flag that differs from its default — an empty object almost always, because the server never sends a value equal to the default; the client replaces its document with it rather than merging key by key. Stations come from the customer's rides of the last 30 days (`from_lat/from_lon` stored at check-in from migration 0019 on) plus the home station; muted stations are excluded server-side. `enabled` is `loc_mode == always && nudge_enabled`, minus a running snooze (docs/24 §3) and minus the 30-day idle switch-off (docs/25 §4) — `idle`, `last_checkin` and `snooze_until` say which of those is folded in.
...
It began as one row of `app_switches` (migration 0037) and is now an ordinary feature flag (#41): a row in `flags`, read from the in-memory snapshot, so `GET /v1/me/geofence` runs no query for it. Migration 0039 moved it and dropped the old table. The flip is `stellwerk --prod flags stations_local on --yes` — no restart. `--yes` is the rail the general system puts in front of a global write; without it the command prints how many people it would reach and writes nothing. `stellwerk switch stations-local on` still works and says the new spelling.

Being a flag rather than a column also means it can be rolled out to a percentage (`--pct`) or set for one person (`--for`), which is why `/v1/me/geofence` carries it rather than the unauthenticated `/v1/flags.json`: only an authenticated request knows who is asking.

**The flip reaches the phone at the next `configure`, which is the next time the app is opened**, not the next time it leaves an umbrella; and `GeofenceSync.sync()` returns early without a logged-in session *and* a healthy backend, so the switch needs a reachable server on both ends and cannot rescue a phone whose backend is down. That bound is not new: `enabled: false` from a running snooze and the idle switch-off already reach the phone only through the same `configure`.
