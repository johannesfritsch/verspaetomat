...
Next: docs/25 (Significant Location Change with an adaptive coverage disc, a region budget that follows the passenger, a 3-minute dwell, a diagnose page), which leans on the docs/24 snooze and rewrites the native layer docs/24's `umbrellaExit` hangs off.
