...
**The log, for the next round of debugging.** Each request records its query, not just its path — `GET /v1/stations/nearby?lat=47.6817&lon=9.8331 → 200 · 91 B · keine Station` rather than `GET /v1/stations/nearby → 200 · 91 B`, which is the line that could not tell "the server found nothing" from "we asked about the wrong place" and cost a round. Alongside it a one-line summary of the answer's *shape* from a short allow-list of endpoints, and, on a failure, the server's own error text, which is short and written for a developer.

**It still holds no bodies.** The summary counts and names stations; it never carries a field value out of a claim, a mail or the passenger's record, and the test suite asserts that an endpoint nobody taught it gets no summary rather than a guess.

**A window rather than everything**: 1, 3, 6, 12, 24 hours or all of it, driving the view *and* the copy — a round of debugging is usually about the last hour, and copying the lot means pasting a week of movements to answer a question about twenty minutes. The copy carries a header of what the phone thinks at that moment: version, backend, permission, mode, the region set with its timestamp and centre, the last nearby lookup, the disc, today's counters and every registered region. The log says what happened; the header says where it ended up, and the first question back was always the second one.

Lines that went wrong are drawn heavier rather than coloured — both reds mean something specific (`app/STYLE.md`) and a diagnostics page does not get to spend a third meaning. A `CLError` is rendered as its reason rather than as Apple's sentence, which is identical for every one of them and thirty words long.
...
**The umbrella is sized from the data, not from a number somebody picked** (issue #31). It used to be a flat 8 km; now its radius reaches to the nearest station the phone is *not* watching, less the station circle, less the distance covered while iOS makes up its mind about an exit. Small in a city, wide in the Allgäu — where a wide one is correct, because there is nothing to miss. The server says how far it searched (`search_radius_m`) and whether that answer is exhaustive (`complete`), and **only `complete` may widen the umbrella past the cautious 8 km**: an empty answer is what a failed lookup, a thin timetable and a bad deploy all look like, and widening on one is how issue #31 sealed itself shut.

Three things this deliberately does not do. It does not shrink by a *percentage* — exit delivery is a time, not a distance, so a tenth of a 1.5 km city umbrella is 150 m where a walker needs 400 m; the margin is absolute. It does not promise that you cannot *reach* an unregistered station, which iOS cannot support at line speed (the phone can be twenty kilometres past the boundary before the exit arrives); what holds is that you cannot *stop* at one without a refresh having been triggered, and stopping is the only case a nudge needs. And it does not exceed what the device will monitor: `maximumRegionMonitoringDistance` had never been read off a phone, and the radius is clamped to nine tenths of it with a log line when that bites.

**The disc and the umbrella are one radius now**, and it took two goes to be true: the first attempt left `configure` writing the speed table's value over the umbrella's on every launch, and `significantUpdate` doing the same for the moment before the lookup — so the two were back out of step on every start, and the log line `radius now 25 km` reported a number that was never used. `discRadius` is now written in exactly one place, `registerUmbrella`. The speed table survives only as the bootstrap value before anything has been registered at all.

**They were two notions of the same reach that could disagree.** They were two notions of the same reach that could disagree, and did: the umbrella-exit path never consulted the disc, so on iOS the 8 km circle silently overrode the 25/60 km speed table on every journey — and Android never had the table at all (`coverageRadius` in `GeofenceManager.kt` has no caller). Johannes' log measured the real cadence at fourteen nearby calls in fifty-nine minutes, against a documented budget of "a day crossing Germany: under 10". That budget has never held and the table was not what was holding it.

**The umbrella is never taken down without being put back.** `configure` starts with `stopAllRegions()` and used to register the umbrella only once a location fix arrived — seconds later, or never, since `requestLocation` can time out and the timeout handler only cleans up. In that window the phone had its station circles and no umbrella, which is the quietest failure this layer has: the page still looks populated, the stations are still there, and nothing re-evaluates ever again. It cost a drive from Kißlegg to München, where the set stayed on Kißlegg for a hundred kilometres and not one line was logged. The umbrella is now re-registered immediately around the last known centre and the fix merely re-centres it — and `status` reports whether it is actually being monitored, so its absence is a row on the page rather than something to infer from silence.

**The umbrella is registered first, and keeps the twentieth slot.** iOS takes twenty regions; `regionSet` now returns at most nineteen, and `refreshNearest` registers the umbrella before the stations. It used to register it last, so under the cap the region iOS refused was the umbrella itself — the one every other trigger hangs off. That never bit only because the server capped the inputs at 15 + 3.

**The nearby list is re-fetched when it belongs somewhere else** (issue #31). `nearest` is persisted in `UserDefaults` and the only thing that ever wrote it was `refreshNearest`, which runs on an umbrella exit, a disc exit, or the button. Standing still none of those can fire — the umbrella and the disc are both centred on the phone — so a list fetched in one town survived every relaunch in the next one, and `configure` re-registered it each time without asking again. That is why Langenargen stayed registered thirty kilometres away in Kißlegg across three builds. The fix stores *where* the list was fetched, and the first fix of a `configure` asks again when the phone is more than `nearestStaleM` (3 km) from that point. Every launch now re-evaluates; movement is no longer required.
...
**This table has never been checked and is known to be wrong.** A log from a real journey on 17 September 2026 shows fourteen `umbrella exit` → `recentred` pairs in fifty-nine minutes — the umbrella fires on its own cadence and consults nothing, and the speed table it was thought to be bounded by is bypassed on iOS and absent on Android. The counters on the Entwicklung page are what will settle the real numbers; until a measured figure replaces these, treat the column as an intention rather than an observation.
