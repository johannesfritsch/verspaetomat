...
- **Opening**: tapping the bar; a check-in completing (Welcher Zug? → the sheet opens on Home automatically); the `/ride` route (pushes, the geofence nudge) opens Home with the sheet. The Unterwegs screen file is reused as the sheet body; the route stays for deep links.
