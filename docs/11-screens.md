...
Navigation (decided 10 September 2026): a bottom bar with four tabs and one action: **Home · Anträge · [Einchecken] · Wir · Ich**. The middle slot is a raised black square with the train icon; it is not a tab but opens the check-in directly (at a station: "Wohin?" for that station, predictions first; elsewhere: the station search, then "Wohin?"). While a journey is under way the square is drawn disabled (grey fill, no shadow) and a tap opens the ride sheet instead of a second check-in (docs/20). Home is the Bahnsteig. Anträge answers "what is happening with my claims?", Wir "what did it all add up to?". The settings gear sits top right on every tab. Nothing is ever called "Spendenkonto": most of what the ledger holds is not a donation yet, and some of it never will be. The claim flow is a full-screen moment that sits on top of the tabs. The ride is not: while a journey is under way, a 56 px **ride bar** sits directly above the bottom nav on every tab (line badge, "nach Rheine", the live delay, the next stop with its time; in transfer the next train, "Umsteigen in Hagen Hbf" and a small ink button "Ich bin drin"), and tapping it opens the **ride sheet** (screen 8) over the active tab. Three hours past the planned arrival with the journey still open, the bar stops reporting and asks instead: "Noch unterwegs?" with "Ich bin da" and "Beenden" side by side, and one push has gone out saying the same (docs/23 §3). Nothing is decided for the passenger: the journey runs until they answer. The bar and the sheet belong to the tab shell (`RideMonitor`, one poll for all tabs); the bar is hidden while the sheet is open. The shell docks the bar above the nav, so tab content is never covered by it (docs/19).
...
- **Buttons:** "Falscher Zug?" (edge state E2) and "Abbrechen". No sharing here; the reveal is at arrival. In demo mode the controls "Nächster Halt" and "Ankunft +68" sit at the bottom of the sheet body.

**"Abbrechen" asks why** (docs/21 §1). A sheet, "Fahrt beenden?", with three choices, each carrying its consequence: **Ich fahre weiter** ("Es zählt die Verspätung bis zum frühesten Zug ab hier. Eine längere Pause zählt nicht mit.", drawn as the recommended one and listed first), **Ich gebe auf** ("Keine Geduldspunkte, kein Anspruch — die Entschädigung hängt an der Ankunft."), **Ich bin gar nicht mitgefahren** ("War ein Versehen. Die Fahrt zählt nirgends mit."), plus "Zurück". Giving up closes with a card, not a snack, because it is the one moment the passenger has a right nobody told them about: the fare back under Art. 18, which with a Deutschlandticket is nothing for the single trip, plus the line "Fährst du doch noch? Dann check wieder ein." Not having travelled at all gets the snack "Fahrt verworfen."
...
**Weiterfahrt** (`transfer_reason: weiterfahrt`, docs/21 §2): the same card with the caption "Weiterfahrt" instead of "Umsteigen", the earliest onward train proposed as usual, and beneath it the line "Frühester Zug ab hier: RB 52, an 10:41 · 38 Minuten zählen." — the ceiling the claim can reach, because a self-chosen pause is the passenger's own time and not the railway's doing. "Anderen Zug wählen" opens Welcher Zug? from this station to the unchanged destination; any itinerary arriving later than the earliest one carries "Deine Pause zählt nicht mit — es bleiben 38 Minuten." Every delay the app shows for such a journey is capped at that number, so the app never promises minutes the claim will not contain.
...
**Nothing at all yet** (no case, no Antrag, nothing taken out) gets the explainer instead of an empty box (docs/21 §5): "Hier wird es später voll.", then the four numbered steps — einchecken, ab 60 Minuten entstehen 1,50 €, ab 4 € geht ein Antrag raus, die Bahn zahlt direkt an den Verein — the caption "Anträge müssen innerhalb eines Jahres gestellt werden. Wir erinnern dich rechtzeitig." and the button "Einchecken".

One card per Antrag, status in words, never dots (docs/18):

- **Wird gesammelt** (first, one per claims desk when there are several): the status line "1,50 von 4,00 €" or "Bereit · 6,00 €" in green, the line "Für Bahnhofsmission Köln · ändern" (the Zweck is visible before there is an Antrag; "ändern" opens the same picker as Einstellungen and changes the customer's setting, docs/20), the incidents, the oldest deadline line (red within 30 days), "Antrag vorbereiten" inside the card when ready. Empty: "Wird gesammelt · Noch nichts · Verspätungen ab 60 Minuten landen hier. Ab 4 € geht ein Antrag raus." Each incident's evidence sheet offers "Nicht einreichen" with three reasons (doch nicht mitgefahren, doppelt erfasst, anderer Grund); a case taken out leaves the bundle and the sum, and the card gains the line "1 Fall nicht eingereicht · anzeigen" with "Doch einreichen" per row (docs/21 §4).
...
- Badges as a grid of Johannes' achievement artwork (`app/assets/achievements/<stem>-aktiv|inaktiv.png`, a red ring with the motif when earned, grey when not; `tools/badge_icons.sh` makes the ground transparent and trims new files), names visible under every tile (the museum is part of the fun). A tap opens the badge sheet with the artwork large, the rule and the date. 20 badges: the 13 moments (first delay, SEV, Stellwerksstörung, …) and the minute ladder 1.000 · 2.000 · 4.000 · 8.000 · 16.000 · 32.000 · 64.000 Verspätungsminuten, awarded by the backend when a ride's finalisation lifts the total past a step. The arrival screen shows a new badge with its artwork, once.
- History ("Alle Fahrten"): every journey, grouped by day and filterable by line, as one row "RE 7 · Köln Hbf → Lüdenscheid · +68" with chips for "2× umsteigen", "Anschluss verpasst", "unvollständig", "aufgegeben", "nicht gefahren" (that row drawn in ink3, and it counts in no statistic). A tap opens the ride in a sheet: its legs with the times they promised and the delay each one brought, the planned and the real arrival, the Geduldspunkte, and at the bottom "Fahrt löschen" (docs/23 §2). That asks once — "Fahrt löschen? Die Fahrt, ihre Punkte und der Anspruch verschwinden. Das lässt sich nicht rückgängig machen." — and then the journey, its legs and its case are gone for good. While the case sits in an Antrag that is already out, the action is shown disabled with the reason beside it, never silently absent. Data: `GET /v1/journeys`, `DELETE /v1/journeys/{id}` (a ride from before journeys existed: `DELETE /v1/rides/{id}`).
