...
final class GeofenceStationsTests: XCTestCase {
  ...
  private static let fixtureURL: URL = {
    ...
    // Measured: `xcodebuild test` forwards a host variable into the simulator's test process only
    // under the `TEST_RUNNER_` prefix — a bare `VERSPAETOMAT_PROBES` never arrives. The bare name
    // is what a scheme's environment or a direct run sets, so both are read and the prefixed one
    // wins. Kotlin's seam is `-Dverspaetomat.probes`.
  ...
  /// The fixture's own header, read tolerantly in the ways it has already changed, and strictly in
  /// the one way it must not.
  ///
  /// Four readers assert against this file and it is regenerated by a fifth program. It has been
  /// written with the CRC in hex and in decimal, with `complete` as `1`/`0` and as `true`/`false`,
  /// and with a column since removed. None of those spellings is worth a red suite — but a fixture
  /// belonging to a *different extract* is, because it is a wrong assertion rather than a looser
  /// one, and after a GTFS import it is the expected state until both are regenerated together.
  func testTheFixtureHeaderIsReadTolerantlyAndTiedToTheExtract() throws {
    let t = try live()
    let dir = try scratchDirectory()
    let original = try String(contentsOf: Self.fixtureURL, encoding: .utf8)
    ...
    // The committed spelling is the baseline.
    let live = try Probes.load(Self.fixtureURL, against: t)
    XCTAssertEqual(live.crc32, t.crc32)
    XCTAssertEqual(live.stationCount, t.count)

    // Hex or decimal: the radix comes from the prefix, and the assertion is on the value.
    let decimal = original.replacingOccurrences(
      of: "crc32=0x\(String(t.crc32, radix: 16))", with: "crc32=\(t.crc32)")
    XCTAssertNotEqual(decimal, original, "the header was not found to rewrite")
    XCTAssertEqual(try Probes.load(try write(decimal, "decimal.tsv"), against: t).crc32, t.crc32)

    // `1`/`0` today, `true`/`false` in an earlier revision.
    let words = original.split(separator: "\n", omittingEmptySubsequences: false).map { line -> String in
      guard !line.hasPrefix("#") && !line.isEmpty else { return String(line) }
      var f = line.split(separator: "\t", omittingEmptySubsequences: false).map(String.init)
      f[4] = f[4] == "1" ? "true" : "false"
      return f.joined(separator: "\t")
    }.joined(separator: "\n")
    let spelled = try Probes.load(try write(words, "words.tsv"), against: t)
    XCTAssertEqual(spelled.rows.map(\.complete), live.rows.map(\.complete))

    // Columns are read by name, so appending one cannot shift the others. A positional parser
    // passes this file and reads the new column as a search radius.
    let widened = original.split(separator: "\n", omittingEmptySubsequences: false).map { line -> String in
      if line.hasPrefix("#"), let names = Probes.columnNamesForTesting(String(line)) {
        return "#   " + (["note"] + names).joined(separator: "  ")
      }
      guard !line.hasPrefix("#") && !line.isEmpty else { return String(line) }
      return "x\t" + line
    }.joined(separator: "\n")
    let moved = try Probes.load(try write(widened, "widened.tsv"), against: t)
    XCTAssertEqual(moved.rows.map(\.radius), live.rows.map(\.radius))
    XCTAssertEqual(moved.rows.map(\.ids), live.rows.map(\.ids))

    // A fixture for another table is refused, by name, with the command that fixes it.
    let stale = original.replacingOccurrences(
      of: "crc32=0x\(String(t.crc32, radix: 16))", with: "crc32=0xdeadbeef")
    XCTAssertThrowsError(try Probes.load(try write(stale, "stale.tsv"), against: t)) { error in
      guard case Probes.Failure.stale(let fixture, let extract)? = error as? Probes.Failure else {
        return XCTFail("expected a staleness refusal, got \(error)")
      }
      XCTAssertEqual(fixture, 0xdead_beef)
      XCTAssertEqual(extract, t.crc32)
      XCTAssertTrue("\(error)".contains("write_the_nearby_probe_fixture"), "the refusal says how to fix it")
    }

    // And a file with no column line is refused rather than guessed at.
    let unnamed = original.split(separator: "\n", omittingEmptySubsequences: false)
      .filter { Probes.columnNamesForTesting(String($0)) == nil }
      .joined(separator: "\n")
    XCTAssertThrowsError(try Probes.load(try write(unnamed, "unnamed.tsv"), against: t)) { error in
      guard case Probes.Failure.noColumnHeader? = error as? Probes.Failure else {
        return XCTFail("expected a missing-column-header refusal, got \(error)")
      }
    }
  }
...
enum Probes {
  ...
  /// Exposed so the parser's own tests can find and rewrite the same line it does, rather than
  /// hardcoding a line number the generator is free to move.
  static func columnNamesForTesting(_ line: String) -> [String]? {
    line.hasPrefix("#") ? columnNames(line) : nil
  }
